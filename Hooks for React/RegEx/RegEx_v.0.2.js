...
const mail = [
    /^[a-zA-Z0-9]+([._-]?[a-zA-Z0-9]+)*@[a-zA-Z0-9-]+\.[a-zA-Z]{2,}(\.[a-zA-Z]{2,})?$/gim, // 0 - Email (не допускает точку в начале и конце, допускает двойные домены)
];
...
const telephone = [
    ...
    /^\+?[1-9]\d{0,2}[-\s]?\(?\d{1,4}\)?[-\s]?\d{1,4}[-\s]?\d{1,9}$/, // 1 - +?[1-9]\d{0,2} — код страны, может начинаться с + [-\s]? — разделители (- или пробел) \(?\d{1,4}\)? — код города в скобках (опционально) [-\s]?\d{1,4}[-\s]?\d{1,9} — основной номер с разделителями. +1-202-555-0123; +44 20 7946 0958; +380 (50) 123-45-67; +7 926 123 4567;
];
...
const ip6 = [
    /^([0-9a-fA-F]{1,4}:){7}([0-9a-fA-F]{1,4})$/,  // 0 - IPv6
];

const password = [
    /^.{8,}$/, // 0 - Минимум 8 символов, abcdefgH, 12345678, !@#ABCdef
    /^\d{8,}$/, // 1 - Только цифры, минимум 8. 12345678, 9876543210
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/,  // 2 - Минимум 8 символов, одна заглавная буква, одна цифра
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_])[a-zA-Z\d\W_]{10,}$/, // 3 - Минимум 10 символов, одна заглавная, одна строчная, одна цифра, один спецсимвол
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_])[a-zA-Z\d\W_]{12,}$/,  // 4 - Минимум 12 символов, одна заглавная, одна строчная, одна цифра, один спецсимвол
];
...
const text = [
    /^[a-zA-Z\s]+$/, // 0 - Разрешает только английские буквы и пробелы (только латиница). 
    /^[a-zA-Z\s-]+$/, // 1 - Разрешает только английские буквы, пробелы и дефисы (только латиница). 
    /^[а-яА-ЯёЁ\s]+$/,  // 2 - Только русские буквы и пробелы
    /^[а-яА-ЯёЁ\s-]+$/, // 3 - Только русские буквы, пробелы и дефисы
    /^[а-яА-ЯёЁіІїЇєЄґҐ\s]+$/, // 4 - Разрешает только украинские буквы и пробелы.
    /^[а-яА-ЯёЁіІїЇєЄґҐ\s-]+$/, // 5 - Разрешает только украинские буквы, пробелы и дефисы.
];
