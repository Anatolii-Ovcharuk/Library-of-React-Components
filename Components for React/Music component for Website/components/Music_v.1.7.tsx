/* "Music component for Website", v. 1.7 - 01.12.2024 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
/* Description: This is TSX Component for React with TypeScript. For Websites with support music on page. */
...
import React, { useState, useEffect, useRef, forwardRef, useImperativeHandle, ForwardedRef, ReactNode, FC } from 'react';
...
const Music = forwardRef((_, ref: ForwardedRef<{ toggleAudio: () => void }>) => {
    ...
    const on_icon: ReactNode | React.ReactNode | JSX.Element = (
    ...
    const off_icon: ReactNode | React.ReactNode | JSX.Element = (
    ...
    const toggleAudio = () => {
        const newPlayState: boolean = !play;
        setPlay(newPlayState);
        localStorage.setItem("musicPlayState", newPlayState.toString());
    };
    ...
    const playAudio = () => {
        if (audioContextRef.current && audioBufferRef.current) {
            ...
            source.loop = true;
            source.loopStart = 0.1; // Change this is value for loop music.
            source.loopEnd = audioBufferRef.current.duration - 0.5; // Change this is value for loop music.
