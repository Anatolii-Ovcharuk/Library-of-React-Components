/* "Console module", v. 1.0 - 30.10.2024 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
...
import React, { Component, useState, useEffect, useContext, useReducer, useMemo, useRef, lazy, Suspense } from 'react';
    ...
    /* INSTALLATION */
/* Add string in application: import ConsoleModule from './Console_v.1.0'; */
...
const ConsoleModule = () => {
        ...
        if (window.confirm("Do you really want to clear the console?")) {
            ...
            const timerOUT = setInterval(() => {
                if (t > 0) {
                    ...
                    setDataConsole('');
                    log("Console was cleared. Version 1.0 - Update: 30.10.2024 | Made by Anatolii Ovcharuk.");
                }
            }, 1000);
        } else {
    ...
    useEffect(() => {
        ...
        if (navigator.clipboard) {
            ...
            setTimeout(() => { 
                console.log("Initializate console ready. Version 1.0 - Update: 30.10.2024 | Made by Anatolii Ovcharuk.");
            }, 500);
            } else {
                // API буфера обмена не доступно
            setTimeout(() => {
                console.log("Initializate console ready, but Clipboard API (Copy) not supported. Version 1.0 - Update: 30.10.2024 | Made by Anatolii Ovcharuk.");
            }, 500);  
        }
    ...
    return (
        <section className='consolePanel'>
            ...
            <pre id="console" className='console' ref={terminalRef} style={{ display: 'block', margin: '10px auto', padding: '10px', width: '75%', height: '150px', boxShadow: 'inset 0px 0px 10px rgba(0,0,0,0.5)', overflow: 'auto' }}>
                {dataConsole}
            </pre>
        </section>
    );
