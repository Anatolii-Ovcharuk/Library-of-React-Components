/* "Theme component", v. 0.7 - 27.07.2025 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
...
import Theme from './components/Theme_v.0.7';
...
const Theme = ({ set_theme = classData }) => {
  ...
  /* EFFECTS */
    ...
    // setIcon(set_theme[indexTheme].name);
    // }, []);

    useEffect(() => {
      const indexTheme = JSON.parse(localStorage.getItem('choiceStyle')) || 0;
      if (set_theme[indexTheme]) {
        setChangeTheme(indexTheme);
        setIcon(set_theme[indexTheme].name);
      } else {
        console.warn("Warning in theme module! set_theme[", indexTheme, "] not finding, set default value (0).");
        setChangeTheme(0);
        setIcon(set_theme[0].name);
      }
    }, []);
...
const handleThemeChange = () => {
  const nextIndex = (currentIndexStyle + 1) % set_theme.length;
  setCurrentIndexStyle(nextIndex);
  setChangeTheme(nextIndex);
  setIcon(set_theme[nextIndex].name);
};

const setChangeTheme = (index) => {
  // const theme = set_theme[index];
  const theme = set_theme?.[index];
  if (!theme || !theme.name) {
    console.error("Тема не найдена или невалидна по индексу:", index);
    return;
  }
