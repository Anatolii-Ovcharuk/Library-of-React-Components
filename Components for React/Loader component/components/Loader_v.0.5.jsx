/* "Loader module", v. 0.5 - 01.12.2024 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
...
import React, { Component, useState, useEffect, useContext, useReducer, useMemo, useRef, lazy, Suspense } from 'react';
    ...
    /* INSTALLATION */
/* Add string in application: import Loader from './Loader_v.0.5' */
...
const Loader = ({ time = TIME_REFRESH, symbol = TYPE_SYMBOL, total = TOTAL_SYMBOL, size = SYMBOL_SIZE, reverse = REVERSE }) => {
    ...
    return (
        <section className='loader' style={{display: "block", margin: "8px auto", width: "90%"}}>
            <p style={{ display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", fontSize: `${size}px`, transition: "opacity linear 250ms, color linear 500ms, fill linear 500ms" }}>
                {
                    Array.from({ length: total }).map((_, index) => (
                        <span key={index} style={{ opacity: count > index ? 1 : 0, transition: "inherit", color: refresh, fill: refresh, textAlign: "center" }}>{symbol}</span>
                    ))
                }
            </p>
        </section>
