/* "Signature module", v. 0.4 - 22.11.2024 React Edition | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
...
const SignatureModule = ({ onSignatureChange }) => {
  ...
  const [currentColor, setCurrentColor] = useState('#0099ff'); // Начальный цвет
  const [lineSize, setLineSize] = useState(2);
  const [savedData, setSavedData] = useState(null); // Хранилище для рисунка
  ...
  useEffect(() => {
    const resizeCanvas = () => {
      if (canvasRef.current) {
        ...
        // Сохраняем данные перед изменением размеров
        ...
        // Восстанавливаем данные после изменения размеров
  ...
  }, [savedData]); // Следим за изменением сохранённых данных
  ...
  return (
    <section ref={formRef}
      ...
      ></canvas>
        ...
        <input style={{width: "50px", margin: "4px", backgroundColor: "inherit", color: "inherit"}} type='number' id="volume" name="volume" min="1" max="9" value={lineSize} onChange={(event) => setLineSize(Number(event.target.value))} /> {/* type="range" */}
      </div>
    </section>
