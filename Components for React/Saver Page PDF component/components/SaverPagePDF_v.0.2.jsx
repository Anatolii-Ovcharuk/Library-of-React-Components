/* "Saver page PDF", v. 0.2 - 05.01.2025 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */
...
const SavePDF = (jsx, orientation_portrait = true, filename = '') => {
  if (!jsx) {
  ...
  const options = {
    ...
    jsPDF: {
      unit: "cm",
      format: "a4", // Указываем фиксированный формат
  ...
  const handleSavePDF = () => {
    if (contentRef.current) {
      const options = {
        ...
        jsPDF: { unit: "cm", format: "a4", orientation: "landscape" } // Landscape orientation (Альбомная ориентация), A4 "a4" - [29.7, 21]
      };
