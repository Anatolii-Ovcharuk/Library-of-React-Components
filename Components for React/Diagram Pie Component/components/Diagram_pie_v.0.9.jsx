/* "Diagram (pie) module", v. 0.9 - 25.05.2025 | MIT License | Made by Anatolii Ovcharuk - https://github.com/Anatolii-Ovcharuk */

    /* INSTALLATION */
/* This .jsx component for react use with file "d3.v7.min.js" or install in project - "npm install d3"  */
/* Add next string in application: import DiagramPie from './Diagram_pie_v.0.9'; */
/* Add next string in application on render or return: <DiagramPie dataset={data} title={"- Total stats in data"} id={"totalData"} /> */
/* Add next string with full options in application on render or return: <DiagramPie dataset={data} title={"- Total stats in data"} id={"totalData"} size={120} font={8} refresh={true} customColors={false} sort={true} inPercentDiagram={true} inPercentLegend={false} /> */
...
const DiagramPieModule = ({ dataset, title = "Diagram", id = "diagram_pie", size, font, refresh, customColors, sort, inPercentDiagram, inPercentLegend, showTotal = true, showLegend = true}) => {
    const [data, setData] = useState(dataset || []);
    let pie, arc, arcs, arcLabel, path, pathlist, svg, color;

    /* OPTIONS */
const SORT_DATA = sort ?? true; /* That's option sort data. Set: True/False */
const REFRESH = refresh ?? true; /* That's option refresh diagram and legend each 1 seconds. Set: True/False */
const SHOW_IN_PERCENT_DIAGRAM = inPercentDiagram ?? true; /* That's option show in diagram data value in percent. Set: True/False */
const SHOW_IN_PERCENT_LEGEND = inPercentLegend ?? false; /* That's option show in diagram data value in legend. Set: True/False */
const CUSTOM_COLORS = customColors ?? false; /* That's option show in diagram and legend customs colors. Set: True/False */
const STROKE_BARS = true; /* That's option show border for bars. Set: True/False */
const DEFAULT_SIZE = 120;
const DEFAULT_FONT_SIZE = 8;
const STROKE_COLOR = '#606060'; /* That's option set border colors. Default value: '#606060'  */
const METRIC_VALUE = '';  /* Add metric for value in legend. Default string is empty - "" */
// Specify the chart’s dimensions.
const width = Number(size) || DEFAULT_SIZE; // 120, 225, 250, 928
const height = width; // Math.min(width, 500), 250
const fontSize = Number(font) || DEFAULT_FONT_SIZE; // Set numbers here in "px";
// CUSTOM COLORS (Array)
...
useEffect(() => {
    ...
    function initialization() {
        ...
        svg.append("g")
            ...
            .call(text => text.append("tspan")
                .attr("class", `pie_text_${id}`)
                .attr("y", "2")
                .attr("font-weight", "bold")
                .attr("font-size", `${fontSize}px`)
                .attr("fill", "#000000")
                .text(d => Number(d.value) !== 0 && d.value ? SHOW_IN_PERCENT_DIAGRAM ? `${d.data.name}: ${(d.data.value / total * 100).toFixed(0)}%` : `${d.data.name}: ${d.data.value.toLocaleString("en-US")}` : null));
        ...
        const legend =  d3.select(`#diagram_pie_legend_${id}`).selectAll(`.legend-item_${id}`)
            ...
            legend.append("p")
                ...
                .attr("class", `legend_item-text_${id}`);
    }
        ...
        textlist.join(
            enter => enter.append("text")
                ...
                .call(text => text.append("tspan")
                    ...
                    .text(d => Number(d.value) !== 0 && d.value ? SHOW_IN_PERCENT_DIAGRAM ? `${d.data.name}: ${(d.value / d3.sum(data.map(d => d.value)) * 100).toFixed(0)}%` : `${d.data.name}: ${d.data.value.toLocaleString("en-US")}` : null)
            ),
            update => update
                .transition().duration(750)
                .attr("transform", d => `translate(${arcLabel.centroid(d)})`)
    ...
    return (
        <section className="section" style={{margin: "10px"}}>
            <h3 className="section__name" style={{textAlign: "center", margin: "10px", fontSize: "14px"}} >{title}</h3>
            <div className="section__diagram" style={{display: "flex", justifyContent: "center", alignItems: "center", gap: "15px", flexWrap: "wrap", flexDirection: "row"}}>
                <div id={`diagram_pie_${id}`} style={{display: "flex", alignItems: "center", justifyContent: "center"}}></div>
                {(showLegend ?? true) && <fieldset className={`field_pie_legend_${id}`} style={{ position: "relative", width: width / 1.25, height: height, margin: "10px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", overflowY: "scroll" }}>
                    <legend style={{ fontSize: `${fontSize * 1.6}px` }} className={`field_pie_legend_text_${id}`}>Legend / Легенда</legend>
                    <div id={`diagram_pie_legend_${id}`} style={{ display: "flex", justifyContent: "center", alignItems: "flex-start", flexWrap: "nowrap", flexDirection: "column", width: "94%", marginTop: "18px" }}></div>
                    {(showTotal ?? true) && <p id={`legend_pie_total_${id}`} style={{ position: "absolute", top: "4px", right: "6px", fontSize: `${fontSize * 1.4}px`, textAlign: `center` }}></p>}
                </fieldset>}
            </div>
